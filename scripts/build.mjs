...
async function build() {
  ...
  let md = "# ModulateJS — markdown for UI interactions\n\nModulateJS is markdown for UI interactions: a consistent language that people and models can both write, for describing complex, fun interface motion. What markdown did for writing, this tries to do for the way an interface moves: a few plain words say what a screen does, and they mean the same thing to a person, to an LLM and to the phone that runs them. It gets an idea out of your head as fast as you can type it without giving up control of the smallest detail. The inspiration is [Strudel](https://strudel.cc), where a few characters are already music; here a few lines are already an app you can hold. This page is a short tour; every word, in full, is in [spec.md](https://modulatejs.com/spec.md). Every example runs as written: paste it into [coral.fm](https://coral.fm/).\n\n";
