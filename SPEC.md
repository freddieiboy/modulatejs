...
| `nav("Inbox", "4 unread")` · `nav("‹ Back", "Film camera")` | the top of a screen, under the safe area: a large title with a dim line under it, or a back chevron with a small title, the way a pushed screen starts. Full width; it sits at the top by itself |
| `people(n = 5)` · `people("Noor Haddad", "Kenji Sato", …)` | rows with a face, a name and a line, a hairline between, full width: an inbox, a contact list. Each row is a layer of its own to tap or drag; the list is a stack |
