...
export const sections = [
  {
    ...
    items: [
      ...
      { verbs: "scroller · a feed", text: "A region that scrolls on its own, under things that don't: momentum, a rubber band, and the same flick landing in the same place every time. feed.scroll drives everything around it; .range(120) is the first 120 points, for a header that shrinks. \"x\", \"page\" (a carousel) and \"both\" are the other ways it goes.", code: `header: card(390, 160).at(0, 0).glass()\nfeed: scroller(stack(20, card())).at(0, 160)\nheader.on(feed.scroll.range(120)).height(88).blur(8)` },
  ...
  },
  {
    ...
    items: [
      ...
      { verbs: "pick · others", text: "One choice that many layers follow. Tapping a member of the group chooses its index; after .on(choice) a layer reads \"<…>\" by that index (pictures crossfade, words fade through), and a group has its chosen member in the other state. others is the rest of a group.", code: `photo: image("tote", 342, 300).at(24, 100)\nname: text("Canvas tote", 26).at(24, 420)\nstrip: row(image("tote", 64, 64), image("mug", 64, 64), image("lamp", 64, 64)).at(24, 480)\nchoice: pick(strip)\nphoto.on(choice).image("<tote mug lamp>")\nname.on(choice).words("<Canvas tote, Stone mug, Paper lamp>")\nstrip.on(choice).scale(1.12).ring("plum")` },
      { verbs: "go · back · stack.depth", text: "A section is a screen. go() shows one on top of what is there (\"cover\", \"push\", \"fade\" or \"sheet\") and remembers it; back() undoes the last one, with everything that changed on the same tap, and so does a swipe in from the left edge. stack.depth is how deep you are.", code: `home: {\n  rows: stack(pill("Canvas tote", 342), pill("Stone mug", 342, "fill")).at(24, 120)\n}\ndetail: {\n  bar: text("‹ Back").at(24, 64)\n  photo: image("tote", 342, 300).at(24, 110)\n}\nrows.on("tap").go(detail, "push")\nbar.on("tap").back()\nhome.on(stack.depth).blur(6)` },
